...
int main(){
    ...
    int m;
    cin >> m;
    bool misma_multitud = (n == m);

    for (int i = 0; i < m; i++) {
        int x;
        cin >> x;
        if (misma_multitud && elementos.has_key(x) && elementos[x] > 0) {
            --elementos[x];
        } else {
            misma_multitud = false;
        }
    }

    cout << (misma_multitud ? "SI" : "NO") << endl;
