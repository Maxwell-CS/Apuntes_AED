...
int main() {
    ...
    int n;
    cin>>n;
    my_map<int, bool> frecuencias_a(2*n);
    for(int i=0;i<n;i++) {
        int x;
        cin>>x;

        frecuencias_a[x] = true;
    }
    int m;
    cin>> m;
    my_map<int, bool> frecuencias_b(2*m);
    for(int i=0;i<m;i++) {
        int x;
        cin>>x;
        if (frecuencias_a.has_key(x) and !frecuencias_b[x]) {
            frecuencias_b[x] = true;
            res.push_back(x);
        }
    }
